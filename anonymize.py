...
def anonymize(ds, anon_path, redcap, PHI_loc = 'top'):
	...
	'''
	COMPLETE ME!!
	This is where you're gonna put some function that will pull your ConvNet generated mask
	-> make sure this generated mask is resized to the current frame size, then assign to the variable 'mask'
	'''
	#############################

	final_mask = np.ones(ds.pixel_array.shape) #the final mask must be the same. Just is just a placeholder
